...
what_i_do = "a script to running NCBI blast automated"
parser = argparse.ArgumentParser(description=what_i_do)
parser.add_argument('-i', dest='input_fasta', type=str, nargs='+',
                required=False, help='input AA fasta file', default=None)
parser.add_argument('-o', dest='output_dir', type=str, nargs=1,
                required=True, help='the output dir (required)', default=None)
args=vars(parser.parse_args())
print args['input_fasta']
out_put_dir = args[output_dir][0]
'''
parser.add_argument('-t', dest='input_table', type=str, nargs=1,
                required=False, help='the output dir(if your file are in table format)', default=None)


for each_file in args['input_table']:
    work_name = each_file.split('.')[0]
    each_out_dir=out_put_dir+'/'+workname
...
#_____________________________________________________________________________________________________________________
for each_file in args['input_fasta']:
    work_name = each_file.split('.')[0]
    each_out_dir=out_put_dir+'/'+workname
    count = 1
    for seq_record in SeqIO.parse(each_file, "fasta"):
        ...
        blast_result_nr = open(os.path.join(each_out_dir,NCBI_blast_result_file_nr),'w')                    
        blast_result_nr.write(blast_handle_nr.read())
        blast_result_nr.close()
        result_handle_nr = open(os.path.join(each_out_dir,NCBI_blast_result_file_nr),'r')
        ...
        all_txt_nr = open(os.path.join(each_out_dir,all_txt_filename_nr),'w')
        title_count = 0
        ...
        if len(list_have_nr) == 0:
            ...
            txt_file_nr = open(os.path.join(each_out_dir,txt_file_name_nr),'w')
            csv_file_nr = open(os.path.join(each_out_dir,csv_file_name_nr),'w')
        ...
        blast_result_swi = open(os.path.join(each_out_dir,NCBI_blast_result_file_swi),'w')                    
        blast_result_swi.write(blast_handle_swi.read())
        blast_result_swi.close()
        result_handle_swi = open(os.path.join(each_out_dir,NCBI_blast_result_file_swi),'r')
        ...
        all_txt_swi = open(os.path.join(each_out_dir,all_txt_filename_swi),'w')
        title_count = 0
        ...
        if len(list_have_swi) == 0:
            ...
            txt_file_swi = open(os.path.join(each_out_dir,txt_file_name_swi),'w')
            csv_file_swi = open(os.path.join(each_out_dir,csv_file_name_swi),'w')
